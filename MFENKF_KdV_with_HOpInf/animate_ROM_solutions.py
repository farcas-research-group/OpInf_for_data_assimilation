...
line1,      = ax.plot(Xg, FOM_data[:, 0], lw=1.50, color=charcoal, label='FOM')
line2,      = ax.plot(Xg, Galerkin_ROM_data[:, 0], lw=1.40, color=color2, label='POD Galerkin ROM (r = {})'.format(r))
line3,      = ax.plot(Xg, OpInf_ROM_data[:, 0], lw=1.40, color=color1, label='H-OpInf ROM (r = {})'.format(r))
...
def update(frame):
    ...
    time_text.set_text(f"t = {frame * speedup* dt:.2f}s") # Update text content
...
ani.save('animations/ROM_solutions.mp4', writer='ffmpeg')