...
def integrate_KdV_ROM_one_step(OpList, r, dt, xHat0, MC=False, 
                          Hamiltonian=True, Newton=True):
    ...
    res = partial(Nfunc, xHat0)
    if not Newton:
        xHat_next = root(res, xHat0, method='krylov').x
    else:
        jac = partial(Nderiv, xHat0)  
        xHat_next = do_newton(res, jac, xHat0, 
                                maxIt=3, sparse=False)
